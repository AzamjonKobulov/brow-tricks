...
const Sidebar = () => {
  ...
  const data = [
    ...
    },
    {
      ...
      path: '/customerspage',
      items: [
        { name: 'Add Customer', path: '/customertablepage' },
  ...
  return (
    <div className="w-[17.125rem] hidden sm:block min-h-screen z-20 bg-brand-greyOne p-2 border-r border-brand-greyFive">
      {data.map((d, idx) => (
              ...
              {isOpen && (
                <div className="absoute top-full flex flex-col">
                  {d.items.map((item) => (
                    <Link
                      to={item.path}
                      className="text-brand-greyNine font-medium pl-10 pt-2"
                    >
                      <p>{item.name}</p>
                    </Link>
                  ))}
